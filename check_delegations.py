...
#         "DNSName": "auditor-prod.tgrrvxy9uk.us-east-1.elasticbeanstalk.com."
#     }, 
#     "Type": "A", 
#     "Name": "auditor.cimpress.cloud."
# }, 
# {
#     "ResourceRecords": [
#         {
#             "Value": "d3q8oskpxt717e.cloudfront.net"
#         }
#     ], 
#     "Type": "CNAME", 
#     "Name": "chat.cimpress.cloud.", 
#     "TTL": 300
# }, 


def check_abandoned_beanstalks(event, context):
    config = load_config('/red-x/')
    r53 = boto3.client('route53')
    zone_id = config['route53']['zoneId']
    ...
    for beanstalk in beanstalks:
        if beanstalk['type'] == 'CNAME':
            print(f"WARN: You should prefer A ALIAS over CNAME for {beanstalk['name']}")
        try:
            answer = dns.resolver.query(beanstalk['value'])
            print(f"OK: {beanstalk['name']}: {', '.join(str(x) for x in answer)}")
        except dns.resolver.NXDOMAIN:
            print(f"CRIT: {beanstalk['name']} points to non-existent beanstalk name: {beanstalk['value']}")
    #     zone = delegation['Name']
    #     nameservers = [d['Value'] for d in delegation["ResourceRecords"]]
